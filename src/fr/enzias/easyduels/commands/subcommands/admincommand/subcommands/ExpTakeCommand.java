...
public class ExpTakeCommand extends SubCommand {

    public ExpTakeCommand(EasyDuels plugin) {
        super(plugin);
    }
            ...
            if (args.length == 4) {
                ...
                if (Bukkit.getPlayer(args[2]) != null) {
                    ...
                    if (levelManager.isValidAmount(args[3])) {
                        levelManager.takeExp(target, levelManager.getValidAmount(args[3]));
                        levelManager.saveExpData(target);
                        sender.sendMessage(messageFile.getTakeExp()
